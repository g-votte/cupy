...
cpdef str _generate_reduction_class_def(
    ...
    private:
      static inline __device__ _type_reduce REDUCE(const _type_reduce& a,
                                                   const _type_reduce& b) {
        return (${reduce_expr});
      }

      static inline __device__ void _REDUCE(_type_reduce* _sdata,
                                            unsigned int tid,
                                            unsigned int offset) {
        if (tid < offset) {
    ...
    public:
      static __device__ void compute(${params_decl}) {
...
cpdef function.Function _get_simple_reduction_kernel(
        ...
        ParameterList param_list, str identity,
...
class simple_reduction_function(object):
    ...
    def __call__(self, a, axis=None, dtype=None, out=None, keepdims=False):
        ...
        kern = _get_simple_reduction_function(
            ...
            self.name, block_size, str(self.identity),
